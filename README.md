...
- **Bar Spectrum** — 低域でうねる3Dバー。時間経過で色相が回転する配色(カラーパレット非対応)。
...
投影窓ごとに、メイン/サブの2色からなるカラーパレットを設定できます。32種類のプリセットから選ぶか、カラーピッカーで自由に配色できます。プリセット選択のドロップダウンは各項目がmain/subの配色で塗り分けられており、選ばなくても一覧から色味を確認できます。パレット非対応のシーン(Bar Spectrum)を選んでいる間は、パレットUIが無効化されます。
