...
- ステータス: Replaced by 015([specs/015-fx-pad.md](015-fx-pad.md)参照。押すと発火するTrigger 1/2/3から、押している位置を連続的に使うFXパッドへ置き換えられ、本仕様のUI・型・実装コードは削除された)
- 作成日: 2026-09-14
...
- 本文中の「`1`/`2`/`3` キー」は要件定義当時の割当。マイクトグルがSpaceからMキーに変わったのに合わせ、後日 Trigger 1=Space、Trigger 2=Cmd右(`MetaRight`)/Ctrl右(`ControlRight`)、Trigger 3=Cmd左(`MetaLeft`)/Ctrl左(`ControlLeft`)に変更された。Windows用にCtrl左右を使っているのは、Winキー(`MetaLeft`/`MetaRight`)がOSのスタートメニュー起動に予約されがちで、ブラウザに`keydown`が安定して届かないため。数字キー自体は本トリガー用途からも外れ、現在は何にも割り当てられていない。
- 本文中「Bar Spectrum・Feedback Loopは今回は非対応(0種類)のままとする」は要件定義当時の記述。後日、Bar Spectrum([02-bar-spectrum.ts](../src/scenes/02-bar-spectrum.ts): Height Kick / Color Flip / White Flash)・Feedback Loop([04-feedback-loop.ts](../src/scenes/04-feedback-loop.ts): Zoom Punch / Flash / Invert)・Wireframe Polyhedron([06-wireframe-polyhedron.ts](../src/scenes/06-wireframe-polyhedron.ts): Spin Kick / Scale Pulse / Flash)・Kaleidoscope([07-kaleidoscope.ts](../src/scenes/07-kaleidoscope.ts): Segment Kick / Spin Burst / Flash)・Bloom Particles([09-bloom-particles.ts](../src/scenes/09-bloom-particles.ts): Radial Burst / Bloom Flash / Freeze)の5シーンにも3種類ずつ追加対応した。Plasma Lava・Grid Terrainは意図的に非対応のまま残している(全シーン対応が目的ではなく、シーンごとに自由に選べる設計を維持するため)。その後、水平の平行線に作り直したRainbow([10-rainbow.ts](../src/scenes/10-rainbow.ts))にも3種類追加対応した: Monochrome(彩度を落としグレースケールへ)/ Pale(白へ寄せて淡く)/ Darken(黒へ寄せて濃く)。
さらにthree.js examplesを参考に追加した4シーンのうち3つにもトリガーを追加対応した(全対応は要件ではないため無理に3種類ずつは埋めていない): Starfield Warp([11-starfield-warp.ts](../src/scenes/11-starfield-warp.ts)) = Warp Speed / Flashの2種、Metaball Blob([12-metaball-blob.ts](../src/scenes/12-metaball-blob.ts)) = Spike / Smoothの2種、Halftone Dots([13-halftone-dots.ts](../src/scenes/13-halftone-dots.ts)) = Invert / Zoom / Flashの3種、Lissajous Lines([14-lissajous-lines.ts](../src/scenes/14-lissajous-lines.ts)) = Ratio Kick / Flashの2種。
その後さらに追加した4シーンにも全てトリガー対応した: Voronoi Cells([15-voronoi-cells.ts](../src/scenes/15-voronoi-cells.ts)) = Shuffle / Flashの2種、Instanced Cube Grid([16-instanced-cube-grid.ts](../src/scenes/16-instanced-cube-grid.ts)) = Height Kick / Wave Pulse / White Flashの3種、Matrix Rain([17-matrix-rain.ts](../src/scenes/17-matrix-rain.ts)) = Speed Burst / Flashの2種、DNA Helix([18-dna-helix.ts](../src/scenes/18-dna-helix.ts)) = Spin Kick / Radius Pulse / Flashの3種。この時点で全18シーン中16シーンがトリガー対応、非対応はPlasma Lava・Grid Terrainの2つのみ。
シーンを全32個まで増やす方針のもと、さらに14シーンを追加しほぼ全てにトリガー対応した: Fireworks(Launch Burst/Flash)・Fresnel Glass Sphere(Glow Burst/Core Flash)・Radial Rays(Burst/Spin)・Aurora(Brighten/Ripple)・Spiral Galaxy(Spin Burst/Flash)・Flocking Boids(Scatter/Flash)・Bouncing Balls(Bounce Burst/Flash)・Chladni Patterns(Mode Shift/Flash)・Sacred Geometry Mandala(Bloom/Spin Burst/Flash)・Lightning Arcs(Strike/Flash)・Glitch Blocks(Corrupt/Flash)・Radial Bar Spectrum(Height Kick/Color Flip/White Flash)・Origami Folding Planes(Fold/Flatten/Flash)・Ribbon Wave(Wave Kick/Flash)。各ファイルパスは[src/scenes/CLAUDE.md](../src/scenes/CLAUDE.md)参照。この時点で全32シーン中30シーンがトリガー対応、非対応は引き続きPlasma Lava・Grid Terrainの2つのみ(全シーン対応が目的ではなく意図的に残している)。
