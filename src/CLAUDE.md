...
- `SceneContextBase.triggers: [number, number, number]` がTrigger 1/2/3それぞれの発生状況(発生時1→指数減衰)を全シーンの `render()` に渡す。`Scene2D`/`SceneWebGL` の `triggerEffectNames?: TriggerEffectNames` に、対応する演出があるインデックスだけ名前を入れる(未対応は `undefined`)。シーンは0〜3個の任意個数だけ対応してよい。現時点の対応シーンは全33シーン中30個(非対応はBlank・Plasma Lava・Grid Terrainのみ、各演出内容は [scenes/CLAUDE.md](scenes/CLAUDE.md) 参照)。
...
| 「Trigger」見出し(Trigger 1/2/3ボタン群の上) | 見出しのみ、操作なし。他セクションと見た目を揃えるために追加([specs/014-ui-status-and-help.md](../specs/014-ui-status-and-help.md)参照) |
...
パネル内の主要項目(Intensity、Crossfade duration、Random、Auto mode、Auto interval、Autoトグル、Trigger、Enable Mic、Add Display)の右には「?」ボタンがあり、押すと共通のヘルプモーダル(`#help-modal`、シーケンス編集モーダルと同じ`.modal-overlay`/`.modal`)にその項目のタイトル・英語の説明文が表示される。各「?」ボタンは`data-help-title`/`data-help`属性に内容を持つだけで、`control.ts`側はクリックされたボタンの`dataset`を`#help-modal-title`/`#help-modal-text`へ流し込む汎用ロジックのみ持つ(項目ごとの個別コードは書いていない)。右上の「✕」ボタン(投影窓の削除ボタンと同じ`.close-btn`)とオーバーレイ自身のクリックの両方で閉じる([specs/014-ui-status-and-help.md](../specs/014-ui-status-and-help.md)参照)。
