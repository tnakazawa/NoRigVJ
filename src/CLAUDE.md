...
- `SceneContextBase.triggers: [number, number, number]` がTrigger 1/2/3それぞれの発生状況(発生時1→指数減衰)を全シーンの `render()` に渡す。`Scene2D`/`SceneWebGL` の `triggerEffectNames?: TriggerEffectNames` に、対応する演出があるインデックスだけ名前を入れる(未対応は `undefined`)。シーンは0〜3個の任意個数だけ対応してよい。現時点の対応シーンは全32シーン中30個(非対応はPlasma Lava・Grid Terrainのみ、各演出内容は [scenes/CLAUDE.md](scenes/CLAUDE.md) 参照)。
