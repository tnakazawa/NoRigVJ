...
| 「Auto mode」ラジオボタン(Random / Sequence) | フルオートの実行内容を選ぶ([specs/013-sequence-mode.md](../specs/013-sequence-mode.md)参照)。`Random`(デフォルト)は下記の挙動、`Sequence`は決めた順にシーンを切り替える。切替自体はフルオートのON/OFF状態に影響しない |
