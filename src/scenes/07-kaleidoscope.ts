...
const fragmentShader = `
  ...
  void main() {
    ...
    // FXパッドX: 分割数への加算オフセット(Segment Kickの連続版)。正で分割が増えて模様が複雑に、
    // 負で分割が減ってシンプルになる(3分割を下限にクランプし、模様が壊れないようにする)
    float segments = max(3.0, floor(5.0 + bass * 6.0 + uPadX * 10.0));
    ...
    // FXパッドY: 明るさへの加算オフセット(Flashの連続版)。正で明るく、負で暗く(0を下回らないようクランプ)。
    // 係数を抑えめにし、少し動かしただけで白飛び/暗転しすぎないようにしている
    float brightness = max(0.0, 0.4 + volume * 0.8 + uPadY * 0.7);
    gl_FragColor = vec4(color * brightness, 1.0);
  }
`;

/** 極座標の角度分割による万華鏡状の反復模様のシーン(WebGL)。カラーパレット対応。FXパッド対応。 */
